...
const RegisterCard = () => {
  ...
  // This runs every time the user types in any field
  const handleChange = (e) => {
    setFormData({ ...formData, [e.target.name]: e.target.value });
    setErrors({ ...errors, [e.target.name]: "" });
  };

  // Validate everything before sending to backend
  const validate = () => {
    const newErrors = {};
    ...
    <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-md z-10 relative">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[#FAD93D] hover:bg-yellow-400 text-gray-800 font-semibold py-2.5 rounded-lg text-sm transition-colors disabled:opacity-60"
        >
